...
// public static EmployeeEncrypted encrypt(EmployeePlain p, ClientKeys keys)
// throws Exception {
// PaillierCrypto paillier = new PaillierCrypto();
...
// e.emailEnc = AESCrypto.encrypt(p.email, keys.aesKey);
// e.contactPhoneNumberEnc = AESCrypto.encrypt(p.contactPhoneNumber,
// keys.aesKey);
// e.personalPhoneNumberEnc = AESCrypto.encrypt(p.personalPhoneNumber,
// keys.aesKey);
...
// e.salaryEnc = AESCrypto.encrypt(String.valueOf(p.salary), keys.aesKey);
// e.salaryPaillier = paillier.encrypt(BigInteger.valueOf(p.salary),
// keys.paillierPublicKey).toString();
...
// String canonical = e.canonicalDataForIntegrity();
// e.recordHmac = IntegrityUtils.hmac(canonical, keys.integrityKey);
// e.recordSignature = ECDSAUtils.sign(canonical,
// keys.ecdsaKeyPair.getPrivate());

// return e;
// }

// public static EmployeeDecrypted decryptAndVerify(EmployeeEncrypted e,
// ClientKeys keys) throws Exception {
// String canonical = e.canonicalDataForIntegrity();

// if (!IntegrityUtils.verify(canonical, e.recordHmac, keys.integrityKey)) {
// throw new SecurityException("Invalid HMAC: encrypted record integrity check
// failed.");
// }

// if (!ECDSAUtils.verify(canonical, e.recordSignature,
// keys.ecdsaKeyPair.getPublic())) {
// throw new SecurityException("Invalid ECDSA signature: encrypted record
...
// d.age = Integer.parseInt(AESCrypto.decrypt(e.ageEnc, keys.aesKey));
// d.email = AESCrypto.decrypt(e.emailEnc, keys.aesKey);
// d.contactPhoneNumber = AESCrypto.decrypt(e.contactPhoneNumberEnc,
// keys.aesKey);
// d.personalPhoneNumber = AESCrypto.decrypt(e.personalPhoneNumberEnc,
// keys.aesKey);
// d.jobTitle = AESCrypto.decrypt(e.jobTitleEnc, keys.aesKey);
// d.departmentID = AESCrypto.decrypt(e.departmentIDEnc, keys.aesKey);
// d.hireDate = AESCrypto.decrypt(e.hireDateEnc, keys.aesKey);
